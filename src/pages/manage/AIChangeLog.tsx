  ...
  return (
    <AppLayout>
      <div className="-m-6 mb-6">
        <PageHero
          title="AI Change Log"
          subtitle="Version history of AI-applied journey changes"
          size="sm"
        />
      </div>
        ...
        {(changes || []).map((change, i) => {
          ...
          return (
            <motion.div key={change.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
              className={`cl-card p-4 ${rolledBack ? 'opacity-60' : ''}`}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {rolledBack && <StatusChip tone="warning">Rolled Back</StatusChip>}
                  </div>
                ...
                {!rolledBack && (
                  <button onClick={() => handleRollback(change)}
                    className="shrink-0 flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border border-border hover:bg-secondary transition-colors text-muted-foreground">
